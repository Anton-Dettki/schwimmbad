...
var testIfShouldAbort = async ($app, eventCopyOrOriginalEvent, originalStart, originalEnd, updateCopy) => {
  ...
  if (onBeforeEventUpdate) {
    ...
    if (!validationResult) {
      updateCopy === null || updateCopy === void 0 ? void 0 : updateCopy(void 0);
      return true;
    }
  }
  return false;
...
var DateGridDragHandlerImpl = class {
  ...
  transformEventCopyPosition(newStartDate) {
    ...
    let daysToShift = Math.round((new Date(newStartDate).getTime() - new Date(dateFromOriginalStart >= this.rangeStartDate ? dateFromOriginalStart : this.rangeStartDate).getTime()) / MS_PER_DAY);
    if (this.$app.config.direction === "rtl")
      daysToShift *= -1;
...
var createDragAndDropPlugin = (minutesPerInterval = 15) => {
