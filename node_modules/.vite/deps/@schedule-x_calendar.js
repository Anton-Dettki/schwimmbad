...
function AppInput() {
  ...
  const [wrapperClasses, setWrapperClasses] = d([]);
  ...
  return u(k, { children: u("div", { className: wrapperClasses.join(" "), id: inputWrapperId, children: [u("label", { for: datePickerInputId, id: datePickerLabelId, className: "sx__date-input-label", children: $app.config.label || $app.translate("Date") }), u("input", { id: datePickerInputId, tabIndex: $app.datePickerState.isDisabled.value ? -1 : 0, name: $app.config.name || "date", "aria-describedby": datePickerLabelId, value: $app.datePickerState.inputDisplayedValue.value, "data-testid": "date-picker-input", className: "sx__date-input", onClick: handleClick, onKeyUp: handleKeyUp, type: "text" }), u("button", { type: "button", tabIndex: $app.datePickerState.isDisabled.value ? -1 : 0, "aria-label": $app.translate("Choose Date"), onKeyDown: handleButtonKeyDown, onClick: () => $app.datePickerState.open(), className: "sx__date-input-chevron-wrapper", children: u("img", { className: "sx__date-input-chevron", src: img, alt: "" }) })] }) });
}
var DatePickerView;
...
function Chevron({ direction, onClick, buttonText, disabled = false }) {
  ...
  return u("button", { type: "button", disabled, className: "sx__chevron-wrapper sx__ripple", onMouseUp: onClick, onKeyDown: handleKeyDown, tabIndex: 0, children: u("i", { className: `sx__chevron sx__chevron--${direction}`, children: buttonText }) });
}
function MonthViewHeader({ setYearsView }) {
  ...
  const [selectedDateMonthName, setSelectedDateMonthName] = d(dateStringToLocalizedMonthName($app.datePickerState.datePickerDate.value));
  const [datePickerYear, setDatePickerYear] = d(getYearFrom($app.datePickerState.datePickerDate.value));
  ...
  return u(k, { children: u("header", { className: "sx__date-picker__month-view-header", children: [u(Chevron, { direction: "previous", onClick: () => setPreviousMonth(), buttonText: $app.translate("Previous month") }), u("button", { type: "button", className: "sx__date-picker__month-view-header__month-year", onClick: (event) => handleOpenYearsView(event), children: selectedDateMonthName + " " + datePickerYear }), u(Chevron, { direction: "next", onClick: () => setNextMonth(), buttonText: $app.translate("Next month") })] }) });
}
function DayNames() {
  const $app = x(AppContext$1);
...
function MonthViewWeek({ week }) {
  ...
  return u(k, { children: u("div", { "data-testid": DATE_PICKER_WEEK, className: "sx__date-picker__week", children: weekDays.map((weekDay) => u("button", { type: "button", tabIndex: hasFocus(weekDay) ? 0 : -1, disabled: !isDateSelectable(weekDay.day), "aria-label": getLocalizedDate($app.datePickerState.datePickerDate.value, $app.config.locale.value), className: weekDay.classes.join(" "), "data-focus": hasFocus(weekDay) ? "true" : void 0, onClick: () => selectDate(weekDay.day), onKeyDown: handleKeyDown, children: weekDay.day.getDate() })) }) });
}
function MonthView({ seatYearsView }) {
  const elementId = randomStringId();
  const $app = x(AppContext$1);
  const [month, setMonth] = d([]);
  const renderMonth = () => {
  ...
  return u(k, { children: u("div", { id: elementId, "data-testid": MONTH_VIEW, className: "sx__date-picker__month-view", children: [u(MonthViewHeader, { setYearsView: seatYearsView }), u(DayNames, {}), month.map((week) => u(MonthViewWeek, { week }))] }) });
}
function YearsViewAccordion({ year, setYearAndMonth, isExpanded, expand }) {
  ...
  return u(k, { children: u("li", { className: isExpanded ? "sx__is-expanded" : "", children: [u("button", { type: "button", className: "sx__date-picker__years-accordion__expand-button sx__ripple--wide", onClick: () => expand(year), children: year }), isExpanded && u("div", { className: "sx__date-picker__years-view-accordion__panel", children: yearWithDates.map((month) => u("button", { type: "button", className: "sx__date-picker__years-view-accordion__month", onClick: (event) => handleClickOnMonth(event, month), children: toLocalizedMonth(month, $app.config.locale.value) })) })] }) });
}
function YearsView({ setMonthView }) {
  ...
  const [expandedYear, setExpandedYear] = d(selectedYear);
  ...
  return u(k, { children: u("ul", { className: "sx__date-picker__years-view", "data-testid": YEARS_VIEW, children: years.map((year) => u(YearsViewAccordion, { year, setYearAndMonth: (year2, month) => setNewDatePickerDate(year2, month), isExpanded: expandedYear === year, expand: (year2) => setExpandedYear(year2) })) }) });
}
var isScrollable = (el) => {
  if (el) {
...
function AppPopup() {
  const $app = x(AppContext$1);
  const [datePickerView, setDatePickerView] = d(DatePickerView.MONTH_DAYS);
  const classList = T(() => {
    const returnValue = [
      POPUP_CLASS_NAME,
  ...
  const [fixedPositionStyle, setFixedPositionStyle] = d(getFixedPositionStyles());
  y(() => {
  ...
  return u(k, { children: u("div", { style: $app.config.teleportTo ? fixedPositionStyle : void 0, "data-testid": "date-picker-popup", className: classList.join(" "), children: datePickerView === DatePickerView.MONTH_DAYS ? u(MonthView, { seatYearsView: () => setDatePickerView(DatePickerView.YEARS) }) : u(YearsView, { setMonthView: () => setDatePickerView(DatePickerView.MONTH_DAYS) }) }) });
}
function AppWrapper({ $app }) {
  const initialClassList = ["sx__date-picker-wrapper"];
  const [classList, setClassList] = d(initialClassList);
  y(() => {
    var _a;
  ...
  return u(k, { children: u("div", { className: classList.join(" "), children: u(AppContext$1.Provider, { value: $app, children: [u(AppInput, {}), $app.datePickerState.isOpen.value && appPopupJSX] }) }) });
}
var AppContext = K({});
...
function RangeHeading() {
  ...
  const [currentHeading, setCurrentHeading] = d("");
  y(() => {
...
function TodayButton() {
  ...
  return u("button", { type: "button", className: "sx__today-button sx__ripple", onClick: setToday, children: $app.translate("Today") });
}
function ViewSelection() {
  const $app = x(AppContext);
  const [availableViews, setAvailableViews] = d([]);
  useSignalEffect(() => {
  ...
  const [selectedViewLabel, setSelectedViewLabel] = d("");
  useSignalEffect(() => {
  ...
  const [viewSelectionItems, setViewSelectionItems] = d();
  const [focusedViewIndex, setFocusedViewIndex] = d(0);
...
function ForwardBackwardNavigation() {
  ...
  const [localizedRange, setLocalizedRange] = d("");
  useSignalEffect(() => {
  ...
  const [rangeEndMinusOneRange, setRangeEndMinusOneRange] = d("");
  const [rangeStartPlusOneRange, setRangeStartPlusOneRange] = d("");
  y(() => {
    const selectedView = $app.config.views.value.find((view) => view.name === $app.calendarState.view.value);
    if (!selectedView)
      return;
  ...
  return u(k, { children: u("div", { className: "sx__forward-backward-navigation", "aria-label": localizedRange, "aria-live": "polite", children: [u(Chevron, { disabled: !!($app.config.minDate.value && dateFromDateTime(rangeEndMinusOneRange) < $app.config.minDate.value), onClick: () => navigate("backwards"), direction: "previous", buttonText: $app.translate("Previous period") }), u(Chevron, { disabled: !!($app.config.maxDate.value && dateFromDateTime(rangeStartPlusOneRange) > $app.config.maxDate.value), onClick: () => navigate("forwards"), direction: "next", buttonText: $app.translate("Next period") })] }) });
}
...
var invokeOnEventClickCallback = ($app, calendarEvent, e) => {
  if ($app.config.callbacks.onEventClick) {
    $app.config.callbacks.onEventClick(calendarEvent._getExternalEvent(), e);
  }
};
var invokeOnEventDoubleClickCallback = ($app, calendarEvent, e) => {
...
function TimeGridEvent({ calendarEvent, dayBoundariesDateTime, isCopy, setMouseDown }) {
  ...
  const startResize = (e) => {
    ...
    if ($app.config.plugins.resize) {
      ...
      $app.config.plugins.resize.createTimeGridEventResizer(eventCopy2, updateCopy, e, dayBoundariesDateTime);
    }
  };
  ...
  }, children: u("div", { "data-ccid": customComponentId, className: "sx__time-grid-event-inner", children: [!customComponent && !hasCustomContent && u(k, { children: [calendarEvent.title && u("div", { className: "sx__time-grid-event-title", children: calendarEvent.title }), u("div", { className: "sx__time-grid-event-time", children: [u(TimeIcon, { strokeColor: eventCSSVariables.iconStroke }), getEventTime(calendarEvent.start, calendarEvent.end)] }), calendarEvent.people && calendarEvent.people.length > 0 && u("div", { className: "sx__time-grid-event-people", children: [u(UserIcon, { strokeColor: eventCSSVariables.iconStroke }), concatenatePeople(calendarEvent.people)] }), calendarEvent.location && u("div", { className: "sx__time-grid-event-location", children: [u(LocationPinIcon, { strokeColor: eventCSSVariables.iconStroke }), calendarEvent.location] })] }), hasCustomContent && u("div", { dangerouslySetInnerHTML: {
    __html: ((_c = calendarEvent._customContent) === null || _c === void 0 ? void 0 : _c.timeGrid) || ""
  } }), $app.config.plugins.resize && !((_d = calendarEvent._options) === null || _d === void 0 ? void 0 : _d.disableResize) && u("div", { className: "sx__time-grid-event-resize-handle", onMouseDown: startResize })] }) }), eventCopy && u(TimeGridEvent, { calendarEvent: eventCopy, isCopy: true, setMouseDown, dayBoundariesDateTime })] });
}
var sortEventsByStartAndEnd = (a, b) => {
  if (a.start === b.start) {
    if (a.end < b.end)
      return 1;
    if (a.end > b.end)
...
var sortEventsForMonthGrid = (a, b) => {
...
var areEvents0MinutesAndConcurrent = (e1, e2) => {
...
var handleEventConcurrency = (sortedEvents, concurrentEventsCache = [], currentIndex = 0) => {
  for (let i = currentIndex; i < sortedEvents.length; i++) {
    ...
    if (concurrentEventsCache.length && (!nextEvent || concurrentEventsCache.every((e) => e.end <= nextEvent.start) && !areBothEventsZeroMinutes)) {
    ...
    if (nextEvent && event.end > nextEvent.start || concurrentEventsCache.some((e) => e.end > event.start) || areBothEventsZeroMinutes) {
      concurrentEventsCache.push(event);
      return handleEventConcurrency(sortedEvents, concurrentEventsCache, i + 1);
    }
...
function TimeGridBackgroundEvent({ backgroundEvent, date }) {
  ...
  return u(k, { children: u("div", { class: "sx__time-grid-background-event", title: backgroundEvent.title, style: {
...
function TimeGridDay({ calendarEvents, date, backgroundEvents }) {
  const [mouseDownOnChild, setMouseDownOnChild] = d(false);
  const $app = x(AppContext);
  ...
  const [eventsWithConcurrency, setEventsWithConcurrency] = d([]);
  y(() => {
  ...
  const [classNames, setClassNames] = d(baseClasses);
  useSignalEffect(() => {
...
function TimeAxis() {
  ...
  return u(k, { children: u("div", { className: "sx__week-grid__time-axis", children: hours.map((hour, index) => u("div", { className: "sx__week-grid__hour", children: [hourCustomComponentFn && hourCCIDs.length && u("div", { "data-ccid": hourCCIDs[index] }), !hourCustomComponentFn && u("span", { className: "sx__week-grid__hour-text", children: formatter.format(new Date(0, 0, 0, hour)) })] })) }) });
}
function DateAxis({ week }) {
  ...
  const weekGridDateCCIDs = d(() => Array.from({ length: 7 }, () => `custom-week-grid-date-${randomStringId()}`));
  y(() => {
  ...
  return u(k, { children: u("div", { className: "sx__week-grid__date-axis", children: week.map((date, idx) => u("div", { className: getClassNames(date), "data-date": toDateString$1(date), children: [weekGridDateCustomComponentFn && u("div", { "data-ccid": weekGridDateCCIDs[0][idx] }), !weekGridDateCustomComponentFn && u(k, { children: [u("div", { className: "sx__week-grid__day-name", children: getDayNameShort(date, $app.config.locale.value) }), u("div", { className: "sx__week-grid__date-number", children: date.getDate() })] })] })) }) });
}
...
function DateGridEvent({ calendarEvent, gridRow, isCopy }) {
  ...
  }, children: [!customComponent && !hasCustomContent && u(k, { children: [hasOverflowLeft && u("div", { className: "sx__date-grid-event--left-overflow", style: overflowStyles }), u("span", { className: "sx__date-grid-event-text", children: [calendarEvent.title, "  ", dateTimeStringRegex.test(calendarEvent.start) && u("span", { className: "sx__date-grid-event-time", children: timeFn(calendarEvent.start, $app.config.locale.value) })] }), hasOverflowRight && u("div", { className: "sx__date-grid-event--right-overflow", style: overflowStyles })] }), hasCustomContent && u("div", { dangerouslySetInnerHTML: {
    __html: ((_c = calendarEvent._customContent) === null || _c === void 0 ? void 0 : _c.dateGrid) || ""
  } }), $app.config.plugins.resize && !((_d = calendarEvent._options) === null || _d === void 0 ? void 0 : _d.disableResize) && !endsAfterWeek && u("div", { className: "sx__date-grid-event-resize-handle", onMouseDown: startResize })] }), eventCopy && u(DateGridEvent, { calendarEvent: eventCopy, gridRow, isCopy: true })] });
}
function DateGridDay({ calendarEvents, date, backgroundEvents }) {
  ...
  return u("div", { className: "sx__date-grid-day", "data-date-grid-date": date, children: [fullDayBackgroundEvent && u("div", { className: "sx__date-grid-background-event", title: fullDayBackgroundEvent.title, style: {
    ...fullDayBackgroundEvent.style
  } }), Object.values(calendarEvents).map((event, index) => {
    if (event === DATE_GRID_BLOCKER || !event)
      return u("div", { className: "sx__date-grid-cell", style: { gridRow: index + 1 }, onMouseDown: handleMouseDown });
    return u(DateGridEvent, { calendarEvent: event, gridRow: index + 1 }, event.start + event.end);
  }), u("div", { className: "sx__spacer", onMouseDown: handleMouseDown })] });
}
...
var WeekWrapper = ({ $app, id }) => {
  ...
  return u(k, { children: u(AppContext.Provider, { value: $app, children: u("div", { className: "sx__week-wrapper", id, children: [u("div", { className: "sx__week-header", children: u("div", { className: "sx__week-header-content", children: [u(DateAxis, { week: Object.values(week).map((day) => toJSDate(day.date)) }), u("div", { className: "sx__date-grid", "aria-label": $app.translate("Full day- and multiple day events"), children: Object.values(week).map((day) => u(DateGridDay, { date: day.date, calendarEvents: day.dateGridEvents, backgroundEvents: day.backgroundEvents }, day.date)) }), u("div", { className: "sx__week-header-border" })] }) }), u("div", { className: "sx__week-grid", children: [u(TimeAxis, {}), Object.values(week).map((day) => u(TimeGridDay, { calendarEvents: day.timeGridEvents, backgroundEvents: day.backgroundEvents, date: day.date }, day.date))] })] }) }) });
};
...
var getWeekNumber = (d3, firstDayOfWeek) => {
  d3 = new Date(Date.UTC(d3.getFullYear(), d3.getMonth(), d3.getDate()));
  const dayOffset = (d3.getUTCDay() - firstDayOfWeek + 7) % 7;
  d3.setUTCDate(d3.getUTCDate() - dayOffset + 3);
  ...
  const weekNo = Math.ceil(((d3.getTime() - yearStart.getTime()) / 864e5 + 1) / 7);
  const nextYearStart = new Date(Date.UTC(d3.getUTCFullYear() + 1, 0, 1));
...
function WeekNumber() {
  const $app = x(AppContext);
  return u("div", { className: "sx__calendar-header__week-number", children: $app.translate("CW", {
...
function CalendarHeader() {
  ...
  const headerContentId = d(headerContent ? randomStringId() : void 0)[0];
  const headerContentLeftPrepend = $app.config._customComponentFns.headerContentLeftPrepend;
  const headerContentLeftPrependId = d(headerContentLeftPrepend ? randomStringId() : void 0)[0];
  const headerContentLeftAppend = $app.config._customComponentFns.headerContentLeftAppend;
  const headerContentLeftAppendId = d(headerContentLeftAppend ? randomStringId() : void 0)[0];
  const headerContentRightPrepend = $app.config._customComponentFns.headerContentRightPrepend;
  const headerContentRightPrependId = d(headerContentRightPrepend ? randomStringId() : void 0)[0];
  const headerContentRightAppend = $app.config._customComponentFns.headerContentRightAppend;
  const headerContentRightAppendId = d(headerContentRightAppend ? randomStringId() : void 0)[0];
  y(() => {
    if (headerContent) {
  ...
  const isDayOrWeekView = T(() => {
    return [viewWeek.name, viewDay.name].includes($app.calendarState.view.value);
  }, [$app.calendarState.view.value]);
  return u("header", { className: "sx__calendar-header", "data-ccid": headerContentId, children: !headerContent && u(k, { children: [u("div", { className: "sx__calendar-header-content", children: [headerContentLeftPrependId && u("div", { "data-ccid": headerContentLeftPrependId }), u(TodayButton, {}), u(ForwardBackwardNavigation, {}), u(RangeHeading, {}, $app.config.locale.value), $app.config.showWeekNumbers.value && isDayOrWeekView && u(WeekNumber, {}), headerContentLeftAppendId && u("div", { "data-ccid": headerContentLeftAppendId })] }), u("div", { className: "sx__calendar-header-content", children: [headerContentRightPrependId && u("div", { "data-ccid": headerContentRightPrependId }), $app.config.views.value.length > 1 && u(ViewSelection, {}, keyForRerenderingOnLocaleChange + "-view-selection"), u(AppWrapper, { "$app": datePickerAppSingleton }), headerContentRightAppendId && u("div", { "data-ccid": headerContentRightAppendId })] })] }) });
}
...
function CalendarWrapper({ $app }) {
  ...
  const [currentView, setCurrentView] = d();
  useSignalEffect(() => {
  ...
  const [previousRangeStart, setPreviousRangeStart] = d("");
  const [transitionClass, setTransitionClass] = d("");
  useSignalEffect(() => {
    var _a2, _b;
  ...
  return u(k, { children: u("div", { className: wrapperClasses.join(" "), id: calendarId, children: u("div", { className: "sx__calendar", children: u(AppContext.Provider, { value: $app, children: [u(CalendarHeader, {}), u("div", { className: ["sx__view-container", transitionClass].join(" "), id: viewContainerId }), $app.config.plugins.eventModal && $app.config.plugins.eventModal.calendarEvent.value && u($app.config.plugins.eventModal.ComponentFn, { "$app": $app }, (_a = $app.config.plugins.eventModal.calendarEvent.value) === null || _a === void 0 ? void 0 : _a.id)] }) }) }) });
}
...
var createDatePickerState = (config2, selectedDateParam) => {
  ...
  const isOpen = d2(false);
  const isDisabled = d2(config2.disabled || false);
  const datePickerView = d2(DatePickerView.MONTH_DAYS);
  const selectedDate = d2(initialSelectedDate);
  const datePickerDate = d2(initialSelectedDate || currentDayDateString);
  const isDark = d2(((_a = config2.style) === null || _a === void 0 ? void 0 : _a.dark) || false);
  const inputDisplayedValue = d2(selectedDateParam ? toLocalizedDateString(toJSDate(selectedDateParam), config2.locale.value) : "");
  const lastValidDisplayedValue = d2(inputDisplayedValue.value);
  E2(() => {
    try {
...
var createCalendarState = (calendarConfig, timeUnitsImpl, selectedDate) => {
  var _a;
  const _view = d2(((_a = calendarConfig.views.value.find((view2) => view2.name === calendarConfig.defaultView)) === null || _a === void 0 ? void 0 : _a.name) || calendarConfig.views.value[0].name);
  const view = w(() => {
    return _view.value;
  });
  const range = d2(null);
  ...
  const isCalendarSmall = d2(void 0);
  const isDark = d2(calendarConfig.isDark.value || false);
  E2(() => {
...
var createCalendarEventsImpl = (events, backgroundEvents, config2) => {
  const list = d2(events.map((event) => {
    return externalEventToInternal(event, config2);
  }));
  const filterPredicate = d2(void 0);
  return {
    list,
    filterPredicate,
    backgroundEvents: d2(backgroundEvents)
  };
};
...
var CalendarConfigImpl = class {
  constructor(locale = DEFAULT_LOCALE, firstDayOfWeek = DEFAULT_FIRST_DAY_OF_WEEK, defaultView = InternalViewName.Week, views = [], dayBoundaries = DEFAULT_DAY_BOUNDARIES, weekOptions, calendars = {}, plugins = {}, isDark = false, isResponsive = true, callbacks = {}, _customComponentFns = {}, minDate = void 0, maxDate = void 0, monthGridOptions = {
    ...
    this.locale = d2(locale);
    this.firstDayOfWeek = d2(firstDayOfWeek);
    this.views = d2(views);
    this.dayBoundaries = d2(dayBoundaries);
    this.weekOptions = d2(weekOptions);
    this.calendars = d2(calendars);
    this.isDark = d2(isDark);
    this.minDate = d2(minDate);
    this.maxDate = d2(maxDate);
    this.monthGridOptions = d2(monthGridOptions);
    this.theme = theme;
    this.translations = d2(translations2);
    this.showWeekNumbers = d2(showWeekNumbers);
...
var createDatePickerConfig = (config2, dateSelectionCallback) => {
  ...
  return new ConfigBuilder().withLocale(config2.locale).withFirstDayOfWeek(config2.firstDayOfWeek).withMin(config2.minDate).withMax(config2.maxDate).withTeleportTo(teleportTo).withStyle((_b = config2.datePicker) === null || _b === void 0 ? void 0 : _b.style).withPlacement(placement).withListeners({ onChange: dateSelectionCallback }).build();
};
...
function MonthGridEvent({ gridRow, calendarEvent, date, isFirstWeek, isLastWeek }) {
  ...
  return u("div", { draggable: !!$app.config.plugins.dragAndDrop, "data-event-id": calendarEvent.id, "data-ccid": customComponentId, onMouseDown: (e) => createDragStartTimeout(handleStartDrag, e), onMouseUp: (e) => setClickedEventIfNotDragging(calendarEvent, e), onTouchStart: (e) => createDragStartTimeout(handleStartDrag, e), onTouchEnd: (e) => setClickedEventIfNotDragging(calendarEvent, e), onClick: handleOnClick, onDblClick: handleOnDoubleClick, onKeyDown: handleKeyDown, className: classNames.join(" "), style: {
    ...
    color: customComponent ? void 0 : eventCSSVariables.color,
    backgroundColor: customComponent ? void 0 : eventCSSVariables.backgroundColor
  }, tabIndex: 0, role: "button", children: [!customComponent && !hasCustomContent && u(k, { children: [dateTimeStringRegex.test(calendarEvent.start) && u("div", { className: "sx__month-grid-event-time", children: timeFn(calendarEvent.start, $app.config.locale.value) }), u("div", { className: "sx__month-grid-event-title", children: calendarEvent.title })] }), hasCustomContent && u("div", { dangerouslySetInnerHTML: {
...
function MonthGridDay({ day, isFirstWeek, isLastWeek }) {
  ...
  const [wrapperClasses, setWrapperClasses] = d(baseClasses);
  y(() => {
  ...
  const monthGridDayNameCCID = d(monthGridDayNameCustomComponent ? randomStringId() : "")[0];
  y(() => {
  ...
  const monthGridDateCCID = d(monthGridDateCustomComponent ? randomStringId() : "")[0];
  y(() => {
  ...
  return u("div", { className: wrapperClasses.join(" "), "data-date": day.date, onClick: (e) => $app.config.callbacks.onClickDate && $app.config.callbacks.onClickDate(day.date, e), "aria-label": getLocalizedDate(day.date, $app.config.locale.value), onDblClick: (e) => {
    var _a, _b;
    return (_b = (_a = $app.config.callbacks).onDoubleClickDate) === null || _b === void 0 ? void 0 : _b.call(_a, day.date, e);
  }, onMouseDown: handleMouseDown, children: [fullDayBackgroundEvent && u(k, { children: u("div", { className: "sx__month-grid-background-event", title: fullDayBackgroundEvent.title, style: {
    ...fullDayBackgroundEvent.style
  } }) }), u("div", { className: "sx__month-grid-day__header", children: [isFirstWeek ? u(k, { children: monthGridDayNameCustomComponent ? u("div", { "data-ccid": monthGridDayNameCCID }) : u("div", { className: "sx__month-grid-day__header-day-name", children: getDayNameShort(dayDate, $app.config.locale.value) }) }) : null, monthGridDateCCID ? u("div", { "data-ccid": monthGridDateCCID }) : u("div", { className: dateClassNames.join(" "), children: dayDate.getDate() })] }), u("div", { className: "sx__month-grid-day__events", children: Object.values(day.events).slice(0, $app.config.monthGridOptions.value.nEventsPerDay).map((event, index) => {
    if (typeof event !== "object")
      return u("div", { className: "sx__month-grid-blocker sx__month-grid-cell", style: { gridRow: index + 1 } });
    return u(MonthGridEvent, { gridRow: index + 1, calendarEvent: event, date: day.date, isFirstWeek, isLastWeek });
  }) }), numberOfNonDisplayedEvents > 0 ? u("button", { type: "button", className: "sx__month-grid-day__events-more sx__ripple--wide", "aria-label": getAriaLabelSingularOrPlural(numberOfNonDisplayedEvents), onClick: handleClickAdditionalEvents, children: `+ ${numberOfNonDisplayedEvents} ${getEventTranslationSingularOrPlural(numberOfNonDisplayedEvents)}` }) : null] });
}
function MonthGridWeek({ week, isFirstWeek, isLastWeek }) {
  const $app = x(AppContext);
  return u("div", { className: "sx__month-grid-week", children: [$app.config.showWeekNumbers.value && u("div", { className: "sx__month-grid-week__week-number", children: getWeekNumber(toJSDate(week[0].date), $app.config.firstDayOfWeek.value) }), week.map((day) => {
    const dateKey = day.date;
    return u(MonthGridDay, { day, isFirstWeek, isLastWeek }, dateKey);
  })] });
}
...
var MonthGridWrapper = ({ $app, id }) => {
  ...
  return u(AppContext.Provider, { value: $app, children: u("div", { id, className: "sx__month-grid-wrapper", children: month.map((week, index) => u(MonthGridWeek, { week, isFirstWeek: index === 0, isLastWeek: index === month.length - 1 }, index)) }) });
};
var config$1 = {
...
function MonthAgendaDay({ day, isActive, setActiveDate }) {
  ...
  return u("button", { type: "button", className: dayClasses.join(" "), onClick: (e) => handleClick(e, $app.config.callbacks.onClickAgendaDate), onDblClick: (e) => handleClick(e, $app.config.callbacks.onDoubleClickAgendaDate), disabled: isBeforeMinDate || isPastMaxDate, "aria-label": getLocalizedDate(day.date, $app.config.locale.value), tabIndex: hasFocus(day) ? 0 : -1, "data-agenda-focus": hasFocus(day) ? "true" : void 0, onKeyDown: handleKeyDown, children: [u("div", { children: jsDate.getDate() }), u("div", { className: "sx__month-agenda-day__event-icons", children: day.events.slice(0, 3).map((event) => u("div", { style: { backgroundColor: `var(--sx-color-${event._color})` }, className: "sx__month-agenda-day__event-icon" })) })] });
}
function MonthAgendaWeek({ week, setActiveDate, activeDate }) {
  const $app = x(AppContext);
  return u("div", { className: "sx__month-agenda-week", children: [$app.config.showWeekNumbers.value && u("div", { className: "sx__month-agenda-week__week-number", children: getWeekNumber(toJSDate(week[0].date), $app.config.firstDayOfWeek.value) }), week.map((day, index) => u(MonthAgendaDay, { setActiveDate, day, isActive: activeDate === day.date }, index + day.date))] });
}
function MonthAgendaDayNames({ week }) {
  ...
  return u("div", { className: classNames, children: localizedShortDayNames.map((oneLetterDayName) => u("div", { className: "sx__month-agenda-day-name", children: oneLetterDayName })) });
}
...
function MonthAgendaEvent({ calendarEvent }) {
  ...
  if (wasEventAddedInLastSecond(calendarEvent))
    classNames.push("is-event-new");
  return u("div", { className: classNames.join(" "), "data-ccid": customComponentId, "data-event-id": calendarEvent.id, style: {
    backgroundColor: customComponent ? void 0 : eventCSSVariables.backgroundColor,
    color: customComponent ? void 0 : eventCSSVariables.color,
    borderInlineStart: customComponent ? void 0 : eventCSSVariables.borderInlineStart,
    padding: customComponent ? "0px" : void 0
  }, onClick: (e) => onClick(e), onDblClick: (e) => onDoubleClick(e), onKeyDown, tabIndex: 0, role: "button", children: [!customComponent && !hasCustomContent && u(k, { children: [u("div", { className: "sx__month-agenda-event__title", children: calendarEvent.title }), u("div", { className: "sx__month-agenda-event__time sx__month-agenda-event__has-icon", children: [u(TimeIcon, { strokeColor: `var(--sx-color-on-${calendarEvent._color}-container)` }), u("div", { dangerouslySetInnerHTML: {
...
var MonthAgendaWrapper = ({ $app, id }) => {
  ...
  const [agendaMonth, setAgendaMonth] = d(getMonth());
  y(() => {
  ...
  return u(AppContext.Provider, { value: $app, children: u("div", { id, className: "sx__month-agenda-wrapper", children: [u(MonthAgendaDayNames, { week: agendaMonth.weeks[0] }), u("div", { className: "sx__month-agenda-weeks", children: agendaMonth.weeks.map((week, index) => u(MonthAgendaWeek, { week, setActiveDate: (dateString) => $app.datePickerState.selectedDate.value = dateString, activeDate: $app.datePickerState.selectedDate.value }, index)) }), u(MonthAgendaEvents, { events: ((_a = agendaMonth.weeks.flat().find((day) => day.date === $app.datePickerState.selectedDate.value)) === null || _a === void 0 ? void 0 : _a.events) || [] }, $app.datePickerState.selectedDate.value)] }) });
};
var config = {
