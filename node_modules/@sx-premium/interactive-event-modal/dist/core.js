...
import { useState, useEffect, useContext, useMemo, useRef as useRef$1 } from 'preact/hooks';
import { Fragment as Fragment$1, createContext, render, createElement } from 'preact';
import { createPortal, useContext as useContext$1, useRef, useMemo as useMemo$1, useEffect as useEffect$1 } from 'preact/compat';
...
const createAppSingleton = (config = {}) => {
    ...
    return new DatePickerAppSingletonBuilder()
        ...
        .withTimeUnitsImpl(timeUnitsImpl)
        .withTranslate(translate(configInternal.locale, signal(translations$1)))
        .build();
};
...
function Textarea({ initialValue, onChange, label, name, errorMessage, placeholder, }) {
    ...
    return (jsx(Fragment, { children: jsxs("div", { class: "sx__form-field", children: [jsxs("div", { className: wrapperClasses.join(' '), children: [jsx("textarea", { className: 'sx__textarea-input', value: inputValue, onInput: (e) => handleInput(e.currentTarget.value), id: "last_name", name: name, placeholder: placeholder }), jsx("label", { className: 'sx__textarea-label', htmlFor: name, children: label })] }), errorMessage && jsx("p", { className: "sx__error-message", children: errorMessage })] }) }));
}
...
function AppPopup() {
    ...
    return (jsxs("div", { className: classList.join(' '), style: $app.config.teleportTo.value ? fixedPositionStyle : undefined, children: [jsx("div", { className: "sx__time-picker-popup-label", children: $app.translate('Select time') }), jsxs("div", { className: "sx__time-picker-time-inputs", children: [jsx(TimeInput, { initialValue: hoursValue, onChange: (newHours) => setHoursValue(newHours), inputRef: hoursRef, nextTabIndexRef: minutesRef, validRange: $app.config.is12Hour.value ? [1, 12] : [0, 23] }), jsx("span", { className: "sx__time-picker-colon", children: ":" }), jsx(TimeInput, { initialValue: minutesValue, onChange: (newMinutes) => setMinutesValue(newMinutes), inputRef: minutesRef, validRange: [0, 59], nextTabIndexRef: OKButtonRef }), $app.config.is12Hour.value && (jsxs("div", { className: "sx__time-picker-12-hour-switches", children: [jsx("button", { type: "button", className: `sx__time-picker-12-hour-switch${$app.timePickerState.isAM.value ? ' is-selected' : ''}`, onClick: () => ($app.timePickerState.isAM.value = true), children: $app.translate('AM') }), jsx("button", { type: "button", className: `sx__time-picker-12-hour-switch${!$app.timePickerState.isAM.value ? ' is-selected' : ''}`, onClick: () => ($app.timePickerState.isAM.value = false), children: $app.translate('PM') })] }))] }), jsxs("div", { class: "sx__time-picker-actions", children: [jsx("button", { type: "button", class: "sx__time-picker-action sx__ripple sx__button-cancel", onClick: () => ($app.timePickerState.isOpen.value = false), children: $app.translate('Cancel') }), jsx("button", { ref: OKButtonRef, type: "button", class: "sx__time-picker-action sx__ripple sx__button-accept", onClick: handleAccept, children: $app.translate('OK') })] })] }));
}
...
function AppComboboxSingleValue({ items, label, initialSelectedItem, onChange, name, isDarkMode, errorMessage, placeholder, }) {
    ...
    return (jsx(Fragment, { children: jsxs("div", { class: "sx__form-field", children: [jsxs("div", { class: "sx__app-combobox-wrapper", id: wrapperId, onClick: toggleOpen, children: [jsxs("div", { class: "sx__app-combobox__input-wrapper", children: [jsx("input", { ref: inputRef, id: inputId, onInput: handleInput, className: "sx__app-combobox-input", type: "text", placeholder: placeholder || 'Search...', name: name }), jsx("label", { className: 'sx__app-combobox-label', htmlFor: name, children: label })] }), createPortal(jsx("div", { id: listId, class: listClasses, style: {
                                top: wrapperRect.y + wrapperRect.height + 1,
                                left: wrapperRect.x,
                                width: wrapperRect.width,
...
const WeekdaySelector = ({ activeDays, onDayClick }) => {
    return (jsx("div", { className: "sx__weekday-selector", children: rruleWeekdays.map((day) => (jsx("button", { className: `sx__day-button ${activeDays.includes(day) ? 'sx__day-button--active' : ''}`, onClick: () => onDayClick(day), children: day }, day))) }));
};
...
function EventForm({ $app, removeEditAddedEvent }) {
     ...
     * */
    const [rruleFreq, setRruleFreq] = useState(() => getInitialFrequencyIncludingCustomOption(modalPlugin, $app).value);
    /**
     * Used for the frequency value of the custom rrule section
     * */
    const [customRRuleFreq, setCustomRRuleFreq] = useState(() => getInitialCustomRRuleFrequency(modalPlugin, $app).value);
    const [rruleUntil, setRruleUntil] = useState(() => getInitialRruleUntil(modalPlugin));
    const [rruleCount, setRruleCount] = useState(() => getInitialRruleCount(modalPlugin));
    const [rruleInterval, setRruleInterval] = useState(() => getInitialRruleInterval(modalPlugin));
    const [rruleEndsType, setRruleEndsType] = useState(() => {
        var _a, _b;
    ...
    const [rruleByday, setRruleByday] = useState(() => getInitialRruleByDay(modalPlugin));
    ...
    return (jsx(Fragment, { children: jsxs("div", { className: "sx__event-form", children: [modalPlugin.isEditingRecurringEvent.value && (jsxs("div", { className: "sx__event-form-alert sx__event-form-recurrence-warning", children: [jsx(WarningSign, {}), $app.translate('You are editing the first in a series of recurring events.')] })), isInHiddenCalendar && (jsxs("div", { className: "sx__event-form-alert sx__event-form-info-box", children: [jsx(InfoIcon, {}), $app.translate('This event is in a hidden calendar.')] })), true !== modalPlugin.config.hideTitle && (jsx(TextField, { id: titleFieldId, initialValue: modalPlugin.formValues.value.title || '', onChange: (value) => {
                        var _a, _b, _c;
                        ...
                        validateIfShouldValidate();
                                            ...
                                            }, min: (_p = modalPlugin.config.datePicker) === null || _p === void 0 ? void 0 : _p.min, max: (_q = modalPlugin.config.datePicker) === null || _q === void 0 ? void 0 : _q.max, initialValue: startDate, isDarkMode: $app.calendarState.isDark.value, locale: $app.config.locale.value }), !isFullDayEvent && (jsx(TimePicker, { "$app": $app, onChange: (newStartTime) => {
                                                var _a, _b, _c;
                                                ...
                                                ? 'bottom-end'
                                                : 'bottom-start', isDarkMode: $app.calendarState.isDark.value, is12Hour: modalPlugin.config.has12HourTimeFormat || false }))] }), jsx("span", { className: 'sx__date-time-until', children: "until" }), jsxs("div", { class: "sx__date-time-picker sx__date-time-end", children: [jsx(DatePicker, { label: ((_w = (_v = modalPlugin.config.fields) === null || _v === void 0 ? void 0 : _v.endDate) === null || _w === void 0 ? void 0 : _w.label) ||
                                            ...
                                            }, initialValue: endDate, isDarkMode: $app.calendarState.isDark.value, min: (_z = modalPlugin.config.datePicker) === null || _z === void 0 ? void 0 : _z.min, max: (_0 = modalPlugin.config.datePicker) === null || _0 === void 0 ? void 0 : _0.max, locale: $app.config.locale.value }), !isFullDayEvent && (jsx(TimePicker, { "$app": $app, onChange: (newEndTime) => {
                                                var _a, _b, _c;
                                                ...
                                                $app.translate(''), initialValue: endTime, placement: isRtl ? 'bottom-start' : 'bottom-end', isDarkMode: $app.calendarState.isDark.value, name: ((_4 = (_3 = modalPlugin.config.fields) === null || _3 === void 0 ? void 0 : _3.endTime) === null || _4 === void 0 ? void 0 : _4.name) || 'end-time', is12Hour: modalPlugin.config.has12HourTimeFormat || false }))] })] }), jsx(AppCheckbox, { label: $app.translate('Full-day event'), initialValue: isFullDayEvent, onChange: (isChecked) => setIsFullDayEvent(isChecked) })] })), showRruleFields && (jsx(Fragment, { children: jsxs("div", { className: 'sx__rrule-wrapper', children: [jsx(AppSelect, { items: getFrequenciesIncludingNoneAndCustom($app), initialSelectedItem: getInitialFrequencyIncludingCustomOption(modalPlugin, $app), label: $app.translate('Frequency'), onChange: (value) => {
                                    setRruleFreq(value);
                                }, name: 'rrule', isDarkMode: $app.calendarState.isDark.value }), rruleFreq === 'custom' && (jsxs("section", { className: 'sx__custom-rrule', children: ["Repeat every \u00A0", jsx("div", { className: 'sx__form-field', children: jsx("div", { class: 'sx__text-field-wrapper', children: jsx("input", { className: 'sx__number-field-input', value: rruleInterval, type: "number", min: 1, max: 1000000, onChange: (e) => {
                                                    const value = parseInt(e.currentTarget.value);
                                                    setRruleInterval(value);
                                                    setRrule();
                                                } }) }) }), "\u00A0 \u00A0", jsx(AppSelect, { items: rruleInterval === 1
                                            ...
                                            setRrule();
                                        }, name: 'rruleFrequency' }, rruleInterval), rruleByday && (jsx(Fragment, { children: jsx(WeekdaySelector, { activeDays: rruleByday, onDayClick: handleClickOnRruleWeekdaySelector }) })), jsxs("div", { className: 'sx__rrule-ends', children: [jsx("h3", { children: "Ends" }), jsx("span", {}), jsxs("div", { className: 'sx__radio', children: [jsx("input", { className: 'sx__radio-input', type: "radio", id: "rrule-never", name: "rrule-ends", value: "never", checked: rruleEndsType === 'never', onChange: () => setRruleEndsType('never') }), jsx("label", { className: 'sx__radio-label', htmlFor: "rrule-never", children: "Never" })] }), jsx("span", {}), jsxs("div", { className: 'sx__radio', children: [jsx("input", { className: 'sx__radio-input', type: "radio", id: "rrule-count", name: "rrule-ends", value: "count", checked: rruleEndsType === 'count', onChange: () => setRruleEndsType('count') }), jsx("label", { className: 'sx__radio-label', htmlFor: "rrule-count", children: "After" })] }), jsxs("div", { children: [jsx("div", { className: 'sx__form-field', children: jsx("div", { class: 'sx__text-field-wrapper', children: jsx("input", { className: 'sx__number-field-input', disabled: rruleEndsType !== 'count', type: "number", min: 1, max: 1000000, value: rruleCount, onChange: (e) => {
                                                                    const value = parseInt(e.currentTarget.value);
                                                                    setRruleCount(value);
                                                                } }) }) }), "\u00A0 occassions"] }), jsxs("div", { className: 'sx__radio', children: [jsx("input", { className: 'sx__radio-input', type: "radio", id: "rrule-until", name: "rrule-ends", value: "until", checked: rruleEndsType === 'until', onChange: () => setRruleEndsType('until') }), jsx("label", { className: 'sx__radio-label', htmlFor: "rrule-until", children: "On" })] }), jsx(DatePicker, { initialValue: rruleUntil, onChange: (newDate) => {
                        ...
                        return (jsxs(Fragment, { children: [field.value.type === 'text' && (jsx(TextField, { id: fieldName, initialValue: modalPlugin.formValues.value[fieldName] || '', onChange: (value) => {
                                        ...
                                        validateIfShouldValidate();
                                    }, placeholder: field.value.placeholder, label: field.value.label || '', fieldName: fieldName, errorMessage: (_a = modalPlugin.formValidationErrors[fieldName]) === null || _a === void 0 ? void 0 : _a.value }, field.value)), field.value.type == 'select' && (jsx(AppSelect, { items: field.value.items || [], label: field.value.label || '', onChange: (value) => {
                                        var _a, _b;
                                        (_b = (_a = modalPlugin.config.customFields[fieldName].value).onChange) === null || _b === void 0 ? void 0 : _b.call(_a, value);
                                        modalPlugin.formValues.value[fieldName] = value;
                                        validateIfShouldValidate();
                                        ...
                                        modalPlugin.formValues.value[fieldName] = value;
                                        validateIfShouldValidate();
                                    }, placeholder: field.value.placeholder, label: field.value.label || '', name: fieldName, errorMessage: (_c = modalPlugin.formValidationErrors[fieldName]) === null || _c === void 0 ? void 0 : _c.value }, field.value)), field.value.type === 'combobox' && (jsx(AppComboboxSingleValue, { items: field.value.items || [], label: field.value.label || '', onChange: (value) => {
                                        var _a, _b;
                                        (_b = (_a = modalPlugin.config.customFields[fieldName].value).onChange) === null || _b === void 0 ? void 0 : _b.call(_a, value);
                                        modalPlugin.formValues.value[fieldName] = value;
                                        validateIfShouldValidate();
                                        ...
                                        return (item.value ===
                                            modalPlugin.formValues.value[fieldName]);
                                    })) || undefined, placeholder: field.value.placeholder, name: fieldName, errorMessage: (_e = modalPlugin.formValidationErrors[fieldName]) === null || _e === void 0 ? void 0 : _e.value }, field.value)), field.value.type === 'combobox-multi' && (jsx(AppCombobox, { items: ((_f = field.value.items) === null || _f === void 0 ? void 0 : _f.map((item) => item.label || '')) ||
                                    ...
                                    field.value.InputComponent && (jsx(field.value.InputComponent, { initialValue: modalPlugin.formValues.value[fieldName] || '', onChange: (value) => {
                                        var _a, _b;
                                        (_b = (_a = modalPlugin.config.customFields[fieldName].value).onChange) === null || _b === void 0 ? void 0 : _b.call(_a, value);
                                        modalPlugin.formValues.value[fieldName] = value;
                                        validateIfShouldValidate();
                                    }, placeholder: field.value.placeholder, label: field.value.label || '', name: fieldName }, field.value))] }));
                    }) })), jsxs("div", { className: 'sx__form-actions', children: [jsx(AppButton, { onClick: saveEvent, additionalClasses: saveButtonClasses.join(' '), children: $app.translate('Save') }), jsx(AppButton, { onClick: cancel, additionalClasses: 'sx__app-button-outlined', children: $app.translate('Cancel') })] })] }) }));
}

function DeleteActions({ $app }) {
    const modalPlugin = $app.config.plugins
        .eventModal;
    return (jsxs("div", { children: [jsx("h2", { className: "sx__modal-delete-headline", children: $app.translate('Are you sure you want to delete this event?') }), jsxs("div", { className: "sx__modal-delete-actions", children: [jsx(AppButton, { additionalClasses: "sx__app-button-outlined", onClick: () => {
                            modalPlugin.modalState.value = 'view';
                        }, children: $app.translate('Cancel') }), jsx(AppButton, { additionalClasses: "sx__app-button-danger", onClick: () => {
                            var _a, _b;
...
export { createInputField, createInteractiveEventModal, rruleFields, translations };
